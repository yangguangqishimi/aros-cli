...
const getEntry = require('./getWebpackFiles');
const readConfig = require('../../../utils/readConfig')
const logger = require('../../../utils/logger')
const webpackConf = require('./webpack.config.new.js')
...
// web base js
gulp.task('build-web-base-js', done => {
    gulp.src('static/js/*')
        .pipe(logger.time('build-web-base-js'))
...
gulp.task("build-js", (callback) => {
    let config = Object.assign({}, webpackConf[0])
    config.entry = getEntry.getEntry();
    config.resolve.alias = getEntry.getAlias();
...
gulp.task("build-js-web", (callback) => {
    let config = Object.assign({}, webpackConf[1])
    config.entry = getEntry.getWebEntry();
    config.resolve.alias = getEntry.getAlias();
    config.plugins = config.plugins.concat(getEntry.generateMultipleEntrys());
    webpack(config).run((err, stats) => {
        process.stdout.write(stats.toString({
        warnings: !!readConfig.get('webpackWarnings'),
          colors: true,
...
gulp.task('dev', (callback) => {
    ...
    gulpSequence('clean', ['build-assets', 'build-iconfont', 'build-js', 'build-js-web'], ['build-web-base-js', 'handle-config-native', "compatible-weex-debug", 'watch', 'socket'])(callback)
});
...
gulp.task('weex-dev', (callback) => {
    gulpSequence('build-js', 'build-js-web', 'compatible-weex-debug', 'socket-send-refresh')(callback)
});
