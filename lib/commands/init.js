...
function create() {
    inquirer.prompt(questions).then(function(answers) {
        ...
        download(`bmfe/${TEMPLATE}`, tmp, function(err) {
            ...
            changeFile(tmp + '/config/aros.native.js', `${TEMPLATE}-name`, name)
            changeFile(tmp + '/config/aros.native.js', `${TEMPLATE}-version`, version)
            changeFile(tmp + '/platforms/android/WeexFrameworkWrapper/gradle.properties', 'com.benmu.wx', applicationID)
            ...
            logger.log('3. aros install')
            logger.log('4. aros dev')
        })
    });
};
