...
var appName = readConfig.get('appName'),
    ...
    bundleUpdate = readConfig.get('erosNativeJs')['url']['bundleUpdate'];
        ...
        promiseAll = files.map(function(item) {
            if (item.indexOf('.zip') !== -1 && item !== jsVersion + '.zip') {
                var md5 = item.slice(0, -4),
                    ...
                    diffZip = path.resolve(process.cwd(), `app/${appName}/jsVersion/${diffZipMd5}.zip`);
...
process.on('message', function(message) {
    var jsVersion = message.jsVersion,
        filePath = path.resolve(process.cwd(), diffpwd, appName);
